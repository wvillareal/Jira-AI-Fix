...
> **Version.** `1.8.0` · source copy — hand-distributed, not unpacked from a release zip
...
> **The instance this gate was built from is in the knowledge base, not here.** Look the object you are about to blame up in `by_object` and read the facts it points at (§1.6e step 5) — on the ticket that produced this gate the writer is another module's deployment sync, and the fact records its body, its input, and the exemption precedent already in it. Two consecutive runs on that ticket concluded *delivery gap* and recommended a patch that the sync reverts on every build. **If the lookup misses, that is R-KB-ABSENCE and not permission to skip the sweep** — the sweep below is what the gate actually requires; the fact only makes it faster.
...
4. **A page is authority only on a branch it is baselined and verified for.** Baselined on the ticket's branch → A1 answers. Baselined elsewhere, or carrying no baseline table at all → the page is a **pointer to confirm against the code**, exactly as a `derived` knowledge-base record is (R-KB-CONFIRM), and **A1 does not answer**. Measured 2026-09-08: **1 page of 3,710** in `RM` carries a baseline table, so *pointer* is the expected outcome today and *authority* is the exception. A page becoming authority is what baselining buys.
...
1. **Version is to an SOP what baselining is to a specification page.** The record carries `strVersionNo`; compare it with `TARGET_BRANCH` as resolved at §1 step 6a. Covering the ticket's line → **A1b answers**. Written against a materially older line → the SOP is a **pointer to confirm against the code**, exactly as an unbaselined page or a `derived` knowledge-base record is (R-KB-CONFIRM), and **A1b does not answer**.
...
| §1.6e module facts of kind `constraint` / `known-gap`, and any `refuted` fact on these objects | a remedy the module requires, and a remedy already tried and found wrong |
...
| **BLAST RADIUS** | who else executes this object, and what breaks if its contract moves? | the caller count, **and it is settled by `git grep` on `TARGET_BRANCH`, never by the index alone**. `by_object` carries `caller_count` / `caller_modules`, but those count **EXEC sites only** and miss every function, view and inline call (R-KB-CALLER-COUNT, §1.6e step 1) — a measured `0` sat on an object with two live consumers. Treat the index number as a floor that tells you where to look, and answer the axis with the grep. A candidate whose callers span other modules inherits §2.6 and §3.7, and that cost belongs in the comparison rather than in a surprise at review |
| **PRECEDENT** | has this module already said how this is done here? | §2.5 `ALIGN_VERDICT` (a sibling body that works is precedent), §1.6e module facts of kind `constraint` and `conventions`, and any `refuted` fact naming this approach. **A `refuted` fact matching a candidate rules that candidate out** — it records an approach that was tried and did not hold, and selecting it anyway needs the same written justification `NOVEL-JUSTIFIED` needs in §2.5 |

**An ABSENT base is not an unresolved constraint (R-MODULE-STANDARD-ABSENCE, measured 2026-09-06).** Read literally, the STOP below plus R-KB-ABSENCE contradict each other: constraint facts live only in the base, so with no base every constraint is permanently "unresolved" and **every data remedy becomes an unconditional STOP**, while R-KB-ABSENCE says a missing base is never a signal and the run proceeds untouched. Both cannot hold. **R-KB-ABSENCE wins:** no base, or a base that returns nothing for these objects, is `MODULE-STANDARD: none found (<base absent | no constraint fact for <objects>>)` and the run **continues**. The STOP below fires only on a constraint that was actually **read and not satisfied** — a rule someone wrote down, not the silence where one might have been.

**Where the module states a standard this runbook does not carry, the module wins (R-MODULE-STANDARD).** The org-wide Data Fix Standards, the Liquibase standard and the acceptance criteria are the **floor**, never the ceiling. IC-29836 passed every check this runbook makes and still failed the module's own rule about lot quantities and valuation. Before selecting a remedy that writes to a table any `constraint` fact names, read that fact and satisfy it, or record `MODULE-STANDARD: <fact> — <how it is satisfied | why it does not apply>`. Where the module has published an addendum (§3.6 PRE-REQ), it is read the same way. **"The module" is the one that OWNS the object being written, not the one in the Jira key (R-KB-WIDEN, §1.6e step 5)** — a remedy that writes another module's table answers to that module's constraint, and its `owner_module` is in the index beside the caller count this gate already reads. **An unresolved module constraint on a table the remedy writes to is a STOP, not a disclosure.**
...
- The policy sources have been read and are applied in full: **Data Fix Standards (All Modules)** (page 705168896 — the baseline this section restates), the **JIRA Datafix Template** (page 434602044), and **the addendum for `MODULE_PREFIX`** where one is published (Accounts Payable's is the **Data Fix Guidelines**, page 503382346). Record which pages were read; where the module has no addendum, record that too rather than silently reading another module's. **A module with no published addendum is not a module with no standard (R-MODULE-STANDARD, §2.7).** Read the knowledge base's facts of kind `constraint` and `conventions` for `MODULE_PREFIX` and for every table this script writes to (§1.6e step 5), and satisfy each or record why it does not apply. Ref IC-29836 — the script passed the template, passed Standards 1–4, and was rejected as *"half baked and did not follow the IC standards"*, because IC requires a lot-quantity change to be checked against valuation data and processes and nothing in the org-wide baseline says so. An unresolved constraint on a table the script writes to is a **STOP**.
  ...
  **Read the OWNING module's facts, not just `MODULE_PREFIX`'s (R-KB-WIDEN, §1.6e step 5).** A data fix that writes another module's table answers to that module's constraint; `owner_module` in `by_object` names it, and each module consulted — including one that held nothing — gets its line in the §6.9 `KB-CONSULTED` block.

  **The STOP fires on a constraint READ AND NOT SATISFIED, never on an absent one (R-MODULE-STANDARD-ABSENCE, §2.7 step 2).** No base on this machine, or no `constraint` fact for these tables, is `MODULE-STANDARD: none found (<base absent | no constraint fact for <tables>>)` and the script **proceeds**. R-KB-ABSENCE governs here exactly as it governs everywhere else: silence where a rule might have been is not an unresolved rule, and reading it as one turns every data fix on a base-less machine into an unconditional STOP.
...
- **§2.7 remedy selection (R-REMEDY-SELECT / R-REMEDY-ONE-CANDIDATE / R-MODULE-STANDARD) — which fix, and why not the others.** Runs after §2.6 and **before §3 authors anything**, on every run that authors, ports or recommends a remedy — data fixes and diagnosis-only Tier-2 runs included; skipped only when the run delivers and recommends nothing. It exists because the graded corpus measured **the mechanism right every time and the remedy wrong or incomplete in four of six delivered fixes**, and no other gate asks the question: §1.6–§2.6 settle the cause, the port and the write permission, and everything after §3 validates a change that already exists. Enumerate at least two candidates at different layers, built from work already done (the §3.4 write chain, the binding or comparand consumer, the §1.7d writer verdict, a §2.5 sibling body, §1.6e `constraint` / `known-gap` / `refuted` facts, surviving §1.6 Step 2a producers) — **a single-candidate list is not an enumeration** unless it records why no other layer can carry the fix. Score each on four axes that are checks rather than opinions: **durability** (does a recurring writer re-assert it — §1.7d), **coverage** (how many sites of the mechanism it closes, the count §3.4 Proof 4 then proves), **blast radius** (caller count, from `derived/_index.json` `by_object` or one `git grep`), **precedent** (a working sibling body, a module `conventions` fact, or a `refuted` fact that rules the candidate out). Record `REMEDY_VERDICT` and **every rejected candidate with the axis that rejected it** — a preference is not an axis. Where a module states a standard this runbook does not carry, the module wins: the org-wide standards are the floor, and an unresolved `constraint` fact on a table the remedy writes to is a **STOP** (ref IC-29836, template-clean and rejected as "half baked and did not follow the IC standards"). Fail-closed on the enumeration, never on the choice: reaching §3 without a verdict is a §3 FAIL, exactly as a missing `ALIGN_VERDICT` is.
